...
const ROOT_DIR = path.join('..', 'loungeware', 'Loungeware');
...
function outputManifest(games) {
  ...
  games.forEach((game) => {
    if (typeof game.config.authors === 'string') {
      game.authorIds = [game.name.split('_')[0]];
    } else {
      game.authorIds = Object.keys(game.config.authors);
    }
  });
